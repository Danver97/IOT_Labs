...
if args.silence is True:
    ...
    total = 9000
    units = 9
    checkpoint_dir = checkpoint_dir + "_silence" 
else:
...
if args.mfcc is True:
    ...
    strides = [2, 1]
    checkpoint_dir = checkpoint_dir + "_mfcc" 
else:
    options = STFT_OPTIONS
    strides = [2, 2]
    checkpoint_dir = checkpoint_dir + "_stft" 
...
model.fit(train_ds, validation_data= val_ds, epochs=20, callbacks=callback_save)
model.summary()

loss, acc = model.evaluate(test_ds)
print(f"TEST: Loss {round(loss,3)}, Accuracy {round(acc,3)}")
with open( checkpoint_dir+"/TEST_result.log", "w" ) as f:
